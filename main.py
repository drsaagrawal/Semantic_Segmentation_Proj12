...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    input = tf.layers.conv2d_transpose(conv_1x1_7, num_classes, 4, 2,padding='same',kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
    ...
    logits = tf.layers.conv2d_transpose(input, num_classes, 16, 8,padding='same',kernel_regularizer=tf.contrib.layers.l2_regularizer(1e-3))
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    cross_entropy_loss += 0.1*l2_loss
    train_op = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(cross_entropy_loss,name="training_op")
...
def run():
    ...
    with tf.Session() as sess:
        ...
        image_input,keep,layer3,layer4,layer7 = load_vgg(sess,vgg_path)
